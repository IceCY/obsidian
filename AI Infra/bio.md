...
主线内核里常见字段可以这么理解。不同内核版本和配置项会有调整，所以这里关注读代码时最常遇到的字段：
...
队列被 stop / quiesce 后又恢复
需要异步 run queue
```

block layer 可能把运行队列的动作放到 workqueue / kblockd 之类的内核线程里。后面这个 worker 再去 run hardware queue，并在它自己的上下文里调用 `queue_rq()`。
...
writeback worker
  -> submit_bio()
  -> request 进入 dispatch list
  -> 暂时没有派发成功

稍后某个 block layer worker
  -> run hw queue
  -> driver queue_rq()
```
...
`blk_mq_ctx` 是 software staging queue。
...
- 多个 CPU 提交 I/O 时减少全局锁竞争
- 更好地保持 cache locality
- 给 merge 和 scheduler 一个软件暂存位置

## `blk_mq_hw_ctx`

`blk_mq_hw_ctx` 常缩写成 `hctx`。

它是 block layer 的 hardware queue context，不一定等于一个真实硬件队列，但通常会映射到驱动的一条提交路径：

```text
NVMe: hctx -> NVMe submission/completion queue pair
virtio-blk: hctx -> virtqueue
SCSI: hctx -> host adapter / hardware queue
SATA/HDD: 可能只有一个或很少几个 hctx
```
...
| 内容 | 说明 |
| --- | --- |
| dispatch list | 已经准备派发但暂时没能发给硬件的 request |
| tags | tag 分配和管理 |
| ctxs | 映射到这个 hctx 的 software queue 集合 |
| queue | 所属 `request_queue` |
| driver_data | 驱动可挂自己的硬件队列上下文 |
| cpumask | 哪些 CPU 可以运行这个 hctx |

## tag

blk-mq 会给 in-flight request 分配 tag。

tag 的作用是：

```text
block layer: 用 tag 标识 request
driver: 可以把 tag 用作硬件 command id 或数组下标
completion: 设备完成时用 tag 快速找到 request
```

这避免了完成路径里线性搜索 request。

NVMe 这类设备天然有 command id / queue id，和 blk-mq tag 模型很贴合。

## I/O scheduler

I/O scheduler 也叫 elevator。

它位于 block layer 中，主要处理 request 的排序、公平性和合并机会。

常见 scheduler：

| scheduler | 典型用途 |
| --- | --- |
| `none` | 尽量少干预，适合 NVMe、虚拟设备或已经有良好调度的设备 |
| `mq-deadline` | 控制读写延迟，避免饥饿，常见默认选择之一 |
| `bfq` | 面向交互体验和公平性的预算公平队列 |
| `kyber` | 用 token 控制读写 / sync async 延迟目标 |
...
注意：调度主要发生在同一个队列上下文内，不是任意 CPU 的所有请求都能无限制全局重排。blk-mq 的设计目标之一就是减少全局锁和跨 CPU cache 抖动。
...
```text
让 block layer 在很短窗口内看到更多相邻 I/O
提高 merge 机会
减少过早派发
```

文件系统 writeback 很常用这种模式。

## merge

如果两个 bio / request 在设备逻辑 sector 上相邻，而且 operation、flags、queue limits、cgroup / priority 等属性兼容，block layer 可以合并：
...
- 减少中断 / completion 开销
- 对 HDD 减少寻道
- 对 SSD / NVMe 减少 per-I/O 软件开销

但合并不是越多越好。对高速设备，过度合并可能降低并行度或增加尾延迟。

## split

如果 bio 不满足设备限制，block layer 会拆分。

常见原因：

- 超过 `max_sectors`
- segment 数超过 `max_segments`
- 单 segment 超过 `max_segment_size`
- 跨 DMA 边界
- 不满足 logical block size / alignment
- discard / write zeroes / zone 边界限制
- stacked device 需要映射到多个下层设备

所以 bio 不一定原样到驱动。

```text
上层提交一个大 bio
  -> block layer 按 queue limits split
  -> 每个 split bio / request 都满足下层限制
```
